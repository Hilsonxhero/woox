...
@section('content')
    ...
    <form id="kt_ecommerce_add_category_form" action="{{ route('admin.categories.update', $category->id) }}" method="post"
                    ...
                    <div class="card-body pt-0">
                        <div class="row">
                            ...
                            <x-input-group class="fv-row col-md-6" label="عنوان">
                                <x-input name="title" placeholder="عنوان" value="{{ old('title') ?? $category->title }}">
                                </x-input>
                            </x-input-group>

                            <x-input-group class="fv-row col-md-6" label="لینک">
                                <x-input name="link" placeholder="لینک" value="{{ old('link') ?? $category->link }}">
                                </x-input>
                            </x-input-group>



                            <x-input-group class="w-100 fv-row col-6" label=" انتخاب دسته والد">
                                <x-select name="parent_id" placeholder="انتخاب دسته والد">
                                    <option value=" ">ندارد</option>
                                    @foreach ($categories as $cat)
                                        <option value="{{ $cat->id }}" @if ($cat->id == $cat->id) selected="selected" @endif>{{ $cat->title }}
                                        </option>
                                    @endforeach
                                </x-select>
                            </x-input-group>
